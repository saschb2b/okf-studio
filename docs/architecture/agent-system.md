...
Pasted sources cross IPC as title and content pairs. Local sources enter through a separate Rust-owned native picker that accepts `.pdf`, `.txt`, `.md`, `.markdown`, `.html`, `.htm`, `.csv`, and `.json`, reads only the selected paths, rejects directories and unsupported content, and returns no absolute path. The webview receives the filename as a bounded origin label and a closed-set media type, not a reusable filesystem grant. HTML is transported as text and never enters a webview HTML sink.
