...
Each new ACP session includes a standard stdio MCP descriptor, which every ACP agent must support. The descriptor starts the current Studio executable in `--okf-mcp <canonical-bundle-root>` mode with an absolute executable path, explicit argv, and no environment entries. That mode parses the bundle once, speaks MCP over inherited stdin/stdout, and exposes only `okf_search` and `okf_traverse`. Search returns at most 50 scored metadata/snippet matches. Traversal is cycle-safe, capped at three breadth-first hops and 200 concepts, and can follow links, backlinks, or both. Outputs are structured and bounded; no write, shell, network, or arbitrary-file tool exists. This helper is mediation, not a sandbox: an already-unsandboxed external process retains its operating-system access.
