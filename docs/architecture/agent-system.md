...
Pasted sources cross IPC as title and content pairs. Rust accepts at most eight, rejects controls in titles and empty content, caps titles at 256 characters, each body at 262,144 characters, and all bodies at 524,288 characters. It converts each pair to a separate labelled ACP text block after bundle context and before the user's final text block. Sources never become paths, grant filesystem access, trigger parsing or network requests, or modify the bundle. The first-turn client notice marks them as untrusted knowledge.
