...
Pasted sources cross IPC as title and content pairs. Local text sources enter through a separate Rust-owned native picker that accepts `.txt`, `.md`, and `.markdown`, reads only the selected paths, rejects directories, non-UTF-8 content, files above 256 KiB, and selections above 512 KiB, and returns no absolute path. The webview receives the filename as a bounded origin label, not a reusable filesystem grant.

Rust accepts at most eight sources, rejects controls in titles and origins and empty content, caps titles at 256 characters, each body at 262,144 characters, and all bodies at 524,288 characters. It recomputes SHA-256 from each submitted body and converts the source to a labelled ACP text block with title, origin, and digest after bundle context and before the user's final text block. Source attachments grant no later filesystem access, trigger no network request, and do not modify the bundle. The first-turn client notice marks them as untrusted knowledge.
