...
| `scan_bundles(folder, max_depth)` / `read_bundle(root)` | Detect bundle roots below an explicit folder, then parse one canonical root into a full [`Bundle`](data-model.md). |
...
- **Least privilege in `capabilities/`.** The Tauri webview capability enables dialog, store, opener, opt-in updater and restart, custom-window controls, and pop-out creation. It grants no direct filesystem or general network API. Rust commands own bundle access, provider requests, source fetches, installations, agent processes, and reviewed transactions. The capability covers the `main` window and `pop-*` reader windows; a pop-out receives the same typed command surface and no extra plugin grant.
