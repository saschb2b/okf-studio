...
The shipped local call can observe ready evidence, empty results, filter mismatch, route-relevant missing metadata, independently sourced conflicts, context-budget omissions, and requested provider failure. It does not label low recall or noisy candidates without query-specific ground truth. It also cannot label generation non-use before an answer and its citations exist. Stale-manifest comparison belongs to retained-receipt replay against a newer fingerprint, not a fresh retrieval over the current manifest.

Those additional classes remain in the versioned schema, so imported evaluations and retained UI states can represent them. Enum presence is not an implementation claim. Studio must not turn an unmeasured score threshold into a user warning merely to populate every class.
...
The spec states that rule outright for one case: a consumer *"SHOULD surface, not silently drop, a failing attestation"* (10.5). For lifecycle it implies it structurally. The spec defines `deprecated` as *"kept for links and history, no longer current"*. A bundle retains a deprecated concept precisely so its links still resolve. A retriever that hid it would break the thing the status exists to preserve.
...
The magnitudes are the substance of the decision, not decoration. An exact id or title match scores 9,000–10,000, so **no demotion here can outrank one**. Search a deprecated concept by name and it still comes back first. What these weights move is the ordering among lexically similar peers. That is the case where "prefer the current one" is right and "hide the old one" is not. `demotion_never_outranks_an_exact_match` in `engine.rs` pins that bound, so a later reweighting cannot quietly turn demotion into exclusion.
...
The engine judges staleness against a date supplied on the request, not a clock read inside the engine. A receipt whose meaning changes on replay is not a receipt. `retrieveOkfContext` defaults that date for every caller, because the failure mode of forgetting it is silent. Nobody notices the staleness, and every test stays green. A caller replaying a historical receipt can still pin its own date.

The engine also reads five older signals: `lifecycle`, `confidence`, `source_class`, `effective_until`, and `review_after`. These are producer conventions taken from `extra` rather than OKF keys. They continue to work, and the engine reads the v0.2 fields alongside them.

# Conflict boundary

Studio raises a conflict caveat conservatively. Different text under a generic heading such as `Why`, `Overview`, or `Decision` is not enough: those sections may describe unrelated concepts. A conflict requires different content from distinct concepts with the same normalized concept title and section heading. Every participating concept must also carry an independent resource or citation identity. This catches separately sourced definitions of the same subject without presenting ordinary variation across the bundle as an error.
...
[Retrieval Intelligence](../features/retrieval-intelligence.md) defines the user-facing behavior. [Retrieval Operations](../product/retrieval-intelligence/retrieval-operations.md) covers rebuild and rollback behavior.
