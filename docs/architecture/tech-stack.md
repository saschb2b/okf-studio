...
- **Components: Base UI (`@base-ui/react`).** Headless, accessible primitives — Dialog, Select, Checkbox, Number Field, Slider, Tooltip, Menu, Scroll Area, and so on — supply behavior, keyboard handling, and ARIA. **Base UI ships no styles**; appearance comes entirely from our [design tokens](../ux/theming.md). This replaces hand-rolled focus traps and native controls for UI conformity and maintenance stability, migrated incrementally (the Settings dialog first).
