...
The assertion that runs after every story is itself tested. `.storybook/visualConsistency.dom.test.ts` covers the checks that read computed style, plus the enforcement list and the failure formatter, and plants a violation for each so a detector that stops detecting fails rather than passing quietly. jsdom performs no layout, so the geometry checks measure zero there and the stories in Chromium remain their only exercise; the test pins that jsdom result so it is not mistaken for a pass. `.storybook/` is linted and typechecked with the rest of the frontend, which it was not while the root `tsconfig` listed it as a bare directory name that its include pattern never matched.
