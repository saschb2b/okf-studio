...
The empty thread states that Studio attaches OKF context on the first send and provides read-only bundle text, search, and link traversal. That first prompt keeps the user's text as its own final block and precedes it with a client-context notice, the canonical OKF skill, specification, commands, templates, and a file resource link to the active bundle's root `index.md`. Agents that advertise embedded-context support receive the four trusted skill documents as Markdown resources. Other agents receive the same documents as labelled text blocks. The bundle index remains a link because bundle content is untrusted knowledge and may change during the session. Later turns do not repeat the context. A failed first prompt retries it.

Every ACP session also receives one stdio MCP server named **OKF Studio**. It exposes only `okf_search` and `okf_traverse`: bounded, structured, read-only queries over the active bundle's parsed concepts and resolved links. Search checks concept identity, type, tags, description, and body without making the agent read every file. Traversal follows links, backlinks, or both for at most three cycle-safe breadth-first hops. Studio declares no additional environment variables for this server and exposes no write tool.
