...
The panel is a labelled complementary region. Streaming uses restrained live regions and never moves focus. Queuing a follow-up moves focus from the disabled Queue control to the queued item's **Edit** action; editing or removing it returns focus to the composer. Tool, diff, attachment, and permission actions are real controls with visible focus. The composer supports multiline input without ambiguous submission.
