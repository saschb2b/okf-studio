...
OKF specialization ships as the built-in `okf-foundation@1.2.0` capability pack. The pack binds the curated skill catalog and the shared Markdown and writing resources. It binds the JSON Schemas for the `okf-artifact-v1` envelope and the `writing-revision-v1` claim ledger. It also binds the complete closed set of Studio tool IDs those skills may require.
...
Before the pack, OKF specialization was spread across skill files, templates, artifact schemas, tool routing, and native code. Those parts could evolve together in source control, but the installed product had no single identity for the method it ran. A user could not inspect the complete unit, compare an upgrade, or return to the earlier behavior without changing the application itself.
...
Named tasks alone left a practical gap. A user who typed “rewrite this bundle” into the ordinary composer could receive only the shared OKF rules. The same request, started from a concept action, received the claim-preserving revision method. Generic chat now discovers the complete catalog so the entry point no longer decides whether domain guidance is available.
...
- declarative Markdown templates and JSON artifact schemas
- required tool IDs from Studio's compiled allowlist.

Unknown fields fail deserialization. The format has no fields for scripts, hooks, binaries, installers, environment values, arbitrary paths, or user-supplied MCP commands. Studio compiles the two capability-discovery tools as read-only tools from the closed allowlist. The pack cannot define another tool. Resources resolve only through compile-time paths below the shipped OKF skill bundle. Both the build script and the runtime repeat the schema, compatibility, path, media-type, conflict, tool, size, and digest checks before the pack becomes active.
...
**Use Legacy 0.4.0** deactivates the pack and leaves only `okf-core`. **Restore OKF Foundation** reactivates the verified pack. Deactivation removes the narrow methods from task and generic-chat routing. It does not delete the immutable resource bytes from the application package. Studio quarantines invalid state and rebuilds it without blocking bundle opening.

The pack receipt stays separate from the app store, custom agent profiles, and native model profiles. It also stays separate from agent-owned session pointers, staged Apply checkpoints, credentials, workspace memory, routine records, and bundle grants. Migration and rollback tests keep representative files from each independent state class byte-for-byte unchanged.
...
The pack builds on [Native OKF Tasks](native-okf-tasks.md), [Structured Agent Work](structured-agent-work.md), and the [Agent System](../architecture/agent-system.md). [OKF Agent Benchmarking](../architecture/agent-benchmarking.md) defines provider limits and evaluation evidence.
