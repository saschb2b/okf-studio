...
- The palette shows the match. It marks the characters a query matched in the result's title. An abbreviation that finds its concept (`grph` matching Graph View) then reads as a hit rather than as a guess. Ranking lives in `features/shell/paletteSearch.ts`, and unit tests cover it. Prefix beats word-start, word-start beats mid-word substring, and mid-word substring beats subsequence. Runs of adjacent characters and matches on word boundaries score higher. A multi-word query requires every term but not their order.
