...
ODSF, the Open Design System Format, is a *profile* of OKF for packaging a design system. An [ODSF](../reference/okf-spec-summary.md) bundle carries design artifacts a plain markdown reader would show only as raw frontmatter and dead links. Those artifacts are machine-readable design tokens, companion HTML and CSS example assets, and design-oriented status. Studio renders them natively in the [Concept Reader](concept-reader.md). Opening an ODSF bundle then *shows* the design system rather than describing it: swatches, type specimens, scales, and (with [`read_asset`](../architecture/ipc-and-security.md)) the live rendered example.
...
|---|---|
| `Color` | a grid of swatches, each with the chip, the token name, and the value. Click a swatch to copy its value. |
| `Typography` | specimens, each role rendered at its own size, weight, and line height |
| `Spacing` | a scale of proportional bars |
| `Shape` | radius sample boxes |
| `Elevation` | shadow sample boxes |
| `Motion` | a duration and easing table |
| anything else with tokens, such as a `Component` | a token table: each token, its raw value, and what it resolves to, with a color dot for color values |
...
A concept's companion example assets render as live previews between the header and the body. The reader shows the actual HTML, rendered, with a Preview and Code toggle and a do-or-don't label. Those assets are `*.example.html` for canonical usage, and the `*.do.html` and `*.dont.html` of a do-and-don't pair. The `examples` frontmatter lists them, and the reader also discovers them from `.html` links in the body. A `# Examples` link is then a live affordance rather than a broken one, and clicking it jumps to the rendered preview.

Studio reads each asset through [`read_asset`](../architecture/ipc-and-security.md). It inlines the stylesheets the asset links, `styles/tokens.css` and `styles/components.css`, so the preview renders truthfully to the system's tokens. The preview is a sandboxed, script-free iframe sized to its content. The asset is static HTML and CSS by spec, so no JavaScript runs. The reader skips a missing asset rather than reporting an error.
