...
- **Layout** — fine-tune the forces in plain terms: *spacing* (how hard nodes push apart), *link length*, *link pull*, *gravity*, and *cluster pull* (how strongly each detected community gathers around its own center). Adjusting one gently reheats the layout so it re-settles.
