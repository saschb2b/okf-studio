...
- This mirrors OKF's intended navigation, where an agent or human "decides where to descend without reading every file". When an `index.md` is missing, Studio synthesizes one from the directory's concepts, which the spec permits. Studio marks it with a quiet *auto* word (tooltip explains) rather than a badge.
...
- **The tree reveals the active concept.** Select from anywhere, a graph node, the [launcher](command-palette.md), or a [reader](concept-reader.md) link. The tree expands the directory chain that leads to that concept, then scrolls its row into view. It always answers "where does this live in the bundle?". It only ever expands. It never re-folds what the user collapsed.
...
- **Peek before you open.** Dwell on a reader link or rail row, or focus it by keyboard. A small **peek card** shows its type, title, description, and first lines. You can judge whether a target is worth opening, or worth a tab, without leaving the page. See the [multi-view proposal](../proposals/multi-view.md).
- Plain clicks keep today's behavior exactly: they navigate the **current tab**, and selection stays one shared state per window.
- **Move to new window** (the strip's trailing control, or the launcher action) undocks a tab into its **own OS window**. That window runs the full app on the same bundle. That is the browser tear-off. Windows are independent, and [live reload](live-reload.md) reaches all of them.
