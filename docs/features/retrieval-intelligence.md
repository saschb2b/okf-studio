...
Every ordinary Agent Panel question can use a Rust-owned retrieval pass over the active granted bundle before it reaches the selected agent. Studio classifies the question, chooses a local route, and compiles coherent sections into a bounded evidence packet. It attaches that packet with stable concept and section identities. The agent does not need its own chunker or unrestricted filesystem access.

The generated attachment carries the lowercase SHA-256 digest of the exact Markdown sent through the prompt boundary. The retrieval receipt remains a separate identity, and Studio never substitutes it for this content digest.
...
Opening the receipt replaces the conversation viewport with an inspector. That inspector answers three user questions: whether the evidence supports the answer, which sources the pass used, and what to do when those sources disagree. The inspector marks conflicting sources directly. Raw ranking, optional provider state, receipt identity, and other implementation detail stay collapsed under **Technical details**. **Search evidence again** states that it updates the evidence view without resending the prompt or rewriting the answer. Studio removes the retained transcript from layout while the inspector is open, so the two surfaces cannot overlap. Closing the inspector restores the conversation, draft, scroll position, and focus.
...
Before this feature, Studio exposed search, graph traversal, and concept reads, but left each agent to decide how to combine them. Two agents could receive different evidence for the same question. A confident wrong answer gave the user no way to tell whether retrieval, filtering, context assembly, or generation had failed. Adding a vector database alone would not solve that trust gap and would make an optional provider part of the product's foundation.

Retrieval Intelligence makes evidence selection a Studio contract. Exact identities remain stronger than similar prose, and authored links can supply bounded relationship context. Tables stay intact, conflicts remain visible, and every omitted candidate has a reason. The result is useful offline and reproducible across agents.

Studio reports optional dense, reranking, long-context, and provider-cache capabilities as configured, degraded, or unavailable. Their absence never disables the local path. Granting remote disclosure does not mark text as shared: the receipt does so only after provider work actually runs.
...
Studio, not the selected agent, chooses the automatic route before sending the prompt. Natural overview, summary, definition, explanation, and change questions have labeled benchmark cases. The receipt explains the resulting method.
...
A retrieval unit carries the bundle fingerprint, concept ID, deterministic section ID, heading path, and source-line range. It also carries the content hash, type, tags, links, backlinks, citations, timestamp and [reliability](reliability-and-lifecycle.md) signals, token estimate, and health caveats. Context compilation keeps units intact instead of cutting them at arbitrary token boundaries. The receipt identity binds filters, limits, provider choices, provider window, and disclosure as well as query, route, budget, and bundle revision. Two materially different searches therefore cannot appear to be the same run.
...
- bundle, manifest, cache-scope, and receipt fingerprints
- dense, reranking, and cache provider states, including whether Studio sent text to a remote provider.
...
The evidence packet requires abstention in three cases. Required authority or current evidence is absent. Independently sourced concepts make different claims about the same subject and section. Included evidence declares a conflict or non-current lifecycle state.

Uncertain confidence produces a caveat without claiming the assertion is objectively unreliable. A missing optional timestamp or reliability field does not affect an ordinary lookup. A time-sensitive answer stays qualified when its concepts provide no timestamp, effective time, or supersession signal. A reused generic heading on unrelated concepts does not count as a conflict.

The flag instructs the agent not to present an unsupported claim as settled. It is not a user error. The conversation translates it into a status such as **Conflicting evidence** or **No supporting evidence**. The inspector and native agent tool expose the bounded caveat. A rank, profile value, or file timestamp never silently becomes authority.
