...
Personal-knowledge tools render markdown and frontmatter and, in Obsidian's case, draw a link graph. But they are not OKF-aware: there is no [type](../reference/okf-spec-summary.md)-colored concept graph, no conformance view, and no notion of a bundle's `index.md` navigation model. They target note-taking in their own vault format, not reading any conformant bundle from any producer. OKF Studio targets the format, not a vault.

# Versus static-site generators (MkDocs, Hugo)

SSGs publish documentation well. They also require a build-and-serve step and produce a static site rather than an interactive [graph](../features/graph-view.md). They offer no live local reload of an arbitrary folder of bundles. They answer "publish these docs for others". OKF Studio answers "let me explore these bundles right now, offline, with no build."
...
A standalone agent can read files and produce text, but the host usually owns the context, permissions, transcript shape, and write behavior. Studio keeps the bundle graph and reader beside the thread, and supplies the canonical OKF skill and closed tools where the connection supports them. It routes proposals through visible staging, conformance validation, hunk review, and transactional apply. The user can change the connected agent without changing that workspace contract.
