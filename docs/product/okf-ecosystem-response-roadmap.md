...
These packages respond to five upstream issues. Stable identity, frontmatter relationships, and rationale trails come from [#120](https://github.com/GoogleCloudPlatform/knowledge-catalog/issues/120). Typed relationships come from [#148](https://github.com/GoogleCloudPlatform/knowledge-catalog/issues/148), graded confidence from [#151](https://github.com/GoogleCloudPlatform/knowledge-catalog/issues/151), maintenance signals from [#158](https://github.com/GoogleCloudPlatform/knowledge-catalog/issues/158), and deletion semantics from [#207](https://github.com/GoogleCloudPlatform/knowledge-catalog/issues/207).
...
| **AP3 Reviewed recipient projection** | A user can choose knowledge and save a separate shareable bundle without editing the source by hand. | Present a two-step choose, then review-and-save flow; keep audience, sensitivity, and exact-redaction controls under optional safeguards; show transitive links, omissions, and link consequences before a native save-location choice. | Source remains unchanged. A blank optional audience filter excludes nothing, unlabeled inclusions are called out, every omitted concept and rewritten link is listed, validation runs on the output, and overwrite requires explicit confirmation. |
...
| AP3 Reviewed recipient projection | Shipped | **Create shareable bundle** now separates content choice from review and save. Advanced handling hints stay under optional safeguards; an empty audience filter excludes nothing; and review leads with what travels, what stays behind, link updates, and text removals. Rust still regenerates the source-bound plan, validates and audits a temporary normal OKF bundle, verifies the unchanged source, and refuses unconfirmed or unmarked replacement. |
...
Related product boundaries: [Design Principles](principles.md), [Scope and Non-Goals](scope-and-non-goals.md), [Validation](../features/validation.md), [Knowledge Health](../features/knowledge-health.md), [Source Adapters and Provenance](../features/source-adapters.md), and [OKF Parsing](../architecture/okf-parsing.md).
