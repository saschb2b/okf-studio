...
4. **Read-only by default.** Pointing the app at a folder never modifies it or starts an agent. Opening and browsing use scoped read access, so inspecting an untrusted bundle remains safe. At launch, the [Agent Panel](../features/agent-panel.md) may visibly reconnect an agent that the user connected and did not disconnect, at most once per session. It never starts an agent the user did not choose. Writing requires an explicit thread grant, a staged revision, validation, human review, and a separate Apply action. See the [Agent System](../architecture/agent-system.md).
