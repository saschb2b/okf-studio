...
- **Keyboard:** `Ctrl/Cmd+T` opens a new tab. The tab opens *empty and active*, and deliberately does not auto-open the launcher. Owner feedback: a dialog popping up unasked reads as a glitch, and the empty state already points at the graph, sidebar, and launcher. Close is `Ctrl/Cmd+W`, and `Ctrl+Tab` and `Ctrl+Shift+Tab` cycle. `Alt+Left` and `Alt+Right` stay per-tab.
