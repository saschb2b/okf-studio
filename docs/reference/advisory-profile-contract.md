...
Studio accepts at most 16 declarations. A namespace uses dot-separated lowercase identifiers. A version is an exact three-part semantic version, not a range. A descriptor path uses forward slashes, ends in `.json`, and contains only normal relative path components. It must resolve inside the authorized bundle, even when symbolic links are present.
...
Checks may apply to all concepts or to the listed `conceptTypes`. Levels are `information`, `recommendation`, and `warning`. Studio retains unknown descriptor keys for inspection and never interprets them. Limits are 128 fields, 128 relationships, and 256 checks. Studio bounds and validates IDs, field paths, labels, descriptions, and messages before a profile becomes active.
...
Profile resolution uses the existing bundle grant and rejects path traversal and symbolic-link escapes. It reads local files only. Descriptor content cannot start network work or code execution. A profile report is a separate diagnostic contract. It never enters `Bundle.issues`, suppresses an OKF issue, hides an unknown field or relationship type, or changes whether the bundle is conformant.
