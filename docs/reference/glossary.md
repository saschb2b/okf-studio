...
- **Cross-link** — A markdown link from one concept to another, asserting a relationship; becomes a directed [graph](../features/graph-view.md) edge.
...
- **Degree** — The number of links touching a concept; used to size [graph](../features/graph-view.md) nodes so well-connected concepts read larger. See also the [data model](../architecture/data-model.md).
...
- **Frontmatter** — The leading `---`-fenced YAML block of a concept. Carries `type` (required) and recommended fields; unknown keys are preserved.
...
- **Persona** — A representative user archetype the product is designed for, used to ground feature decisions. See [Personas](../product/personas.md).
