...
OKF (Open Knowledge Format) v0.1 is Google's vendor-neutral spec for the context an AI agent needs. The spec represents that context as a **bundle**: a directory of markdown **concept** files with YAML frontmatter, kept in version control. It is the format OKF Studio reads. This is a condensed snapshot for implementers. The canonical spec is the `resource` above.
...
- **`index.md`**: a directory listing for progressive disclosure. It carries no frontmatter, except the bundle-root `index.md`, which may declare `okf_version`.
