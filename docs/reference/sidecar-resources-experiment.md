...
Rust canonicalizes each path inside the granted bundle and refuses symlink escapes. It caps inspection and export at 64 MiB, computes SHA-256, and compares an optional authored digest. A closed safe-preview media set may use existing inert readers. Everything else remains download-only.
