...
- **Capabilities & permissions (v2):** access is denied by default; a `capabilities/` config grants specific permissions scoped to windows. Studio grants the webview dialog, store, opener, updater, and window capabilities, but no filesystem plugin or general network capability. Typed Rust commands own filesystem, fetch, and agent operations behind the [IPC boundary](../architecture/ipc-and-security.md).
