...
The turn fixture now groups one user prompt with its ordered prose, tool calls, plan, and result. It retains interleaving around tool calls, exposes one **Copy response** action for the complete turn, and shows **Copy selection** only for an active selection inside that turn. Successful detailed tools and completed plans are collapsed by default; running and failed activity remains open. Read activity stays a compact row, and a title that already names its concept path does not repeat the same path beside it.
...
- **Untidy, Safe, fixed:** successful command output, edit diffs, and completed plans remained open after they stopped needing attention. Their resting state is now collapsed; running and failed work stays open.
