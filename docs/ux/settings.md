...
The [Bundle Switcher](../features/bundle-switcher.md) manages recent bundles, where they are part of the context-switching job rather than a global preference.
...
[Speed reading](../features/speed-reading.md) keeps its pace, frame size, and word-start cue in the same store. Its *mode* is deliberately absent from that store. Text that advances on its own has to follow a press every time, so no setting can leave a player armed for the next concept.
...
Notification sound is available only after the user enables notifications. The operating system decides whether sound plays and whether focus suppresses it. Permission failures remain beside these controls with a direct explanation.
...
Knowledge settings require an open bundle because each item binds to one Rust-granted root. Without a bundle, the category explains that requirement instead of showing disabled controls without context.
...
Installing or downloading a release is always an explicit user action. Once shortly after launch, Studio quietly reads GitHub's release manifest to learn whether a newer version exists. The only surface of that check is a small warning-colored dot. It sits on the Settings icon in the [Activity Bar](browsing-layout.md) and on the Updates category inside this workspace. A new release therefore gets noticed without a dialog, toast, or sound.

The dot animates once on arrival and then rests still. Reduce motion removes the animation. A failed or offline check reports nothing, so an offline launch looks identical to an up-to-date one. The New release badge setting turns the launch check off entirely, restoring strictly on-demand checking.
...
Reset to defaults restores the shipped preference values. Those are System theme, default reader size, scan depth 8, motion enabled, the new release badge on, and agent notifications and sound off. It does not delete bundle memory, routines, profiles, sessions, credentials, checkpoints, or grants.
