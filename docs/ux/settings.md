...
- **OKF capability pack.** The `okf-foundation@1.1.0` card shows built-in provenance, Studio and schema compatibility, conflicts, templates, artifact schemas, required tools, pack digest, and activation state before the per-capability versions and resources. The catalog includes the shared writing contract plus concept-authoring and meaning-preserving revision capabilities. **Use Legacy 0.3.0** retains only `okf-core`; **Restore OKF Foundation** returns the curated task set. Either choice leaves profiles, sessions, checkpoints, settings, and grants unchanged. This is disclosure, not a claim that an external agent followed delivered guidance; see [Declarative OKF Capability Packs](../features/capability-packs.md).
