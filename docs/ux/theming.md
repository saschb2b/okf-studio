...
Zed splits these the same way (`element.*` and `ghost_element.*`), and for the same reason: the translucent pair composites onto whatever is behind it, so a row hovers correctly on all four surfaces without knowing which one it sits on.
...
- Dark keeps the app's near-black identity — surfaces sit around 7–16% lightness rather than Zed's 18–26% — but the four steps are spaced evenly enough that a pane edge is visible without leaning on `--border` for every seam.
...
Every text-carrying role clears WCAG AA (4.5:1) on all four surfaces **and on the state fills stacked on them**. That last clause is new, and it is what a hand-check kept missing: the roles were verified against the resting surfaces only, and a hovered row is a surface too. Colored ink on a hovered row inside a dialog was the worst case, and it failed — `--accent` on `--ghost-active` over `--bg-overlay` came to 3.58 in dark. Fixing it moved the dark accent up and the light accent, error, and warn down.

`pnpm check:contrast` re-derives all 90 pairings per theme straight from the declarations in `src/styles.css` and fails on any below 4.5. Run it before changing a color role, and check both directions: a role has to be readable *as* ink and to take `--accent-contrast` at 4.5 when used as a *fill*. Disabled text is the one exception — WCAG 1.4.3 exempts it, and `--text-disabled` does not clear the bar.
