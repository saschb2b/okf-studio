...
The text roles are `--text` and `--text-dim`. The accent roles are `--accent` plus `--accent-contrast`, `--accent-hover`, and `--accent-active`. The status roles are `--error` (with its own hover/active pair, since it is a fill on the danger button and the window close box), `--warn`, and `--ok`. Each of accent, error, warn, and ok also has a translucent `-soft` tint for selected rows, callouts, and status chips. The tint is alpha rather than a mixed-in surface color, so one token works on every surface.
