...
pub(crate) fn native_system_message() -> LocalChatMessage {
    ...
    LocalChatMessage {
        role: "system",
        content: format!(
            "{SYSTEM_INTRODUCTION}\n\n{NATIVE_BOUNDARY}\n\nAvailable skill catalog (metadata only; detailed instructions are not preloaded):\n- okf: {description}\n\nUse `{LOAD_SKILL_RESOURCE_TOOL}` to load only the detailed OKF resource needed for the task. Start with `instructions`; load `specification`, `commands`, or `templates` only when relevant. Loading a skill resource does not grant bundle, source, filesystem, validation, or write access. Do not claim to have applied guidance you did not load. You may still explain general concepts from the conversation, clearly separating them from facts about the active bundle."
        ),
    }
}
